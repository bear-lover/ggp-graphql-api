...
const getBillingCalculation = {
	...
	async resolve({ request }, { listId, startDate, endDate, guests, convertCurrency }) {
		...
		let stayedNights = [], currentDay, isSpecialPriceAssigned = false, breakPoint, priceIncludeTotal = 0, cleaningPrice = 0;
			...
			if (isMaxNight || isMinNight) {
				...
				if (isBlocked && isBlocked.length > 0) {
					...
					if (listingData) {
						...
						priceIncludeTotal = (priceForDays + cleaningPrice) - discount;

						if (serviceFees) {
							hostServiceFeeType = serviceFees.hostType;
							hostServiceFeeValue = serviceFees.hostValue;
							if (serviceFees.hostType === 'percentage') {
								hostServiceFee = priceIncludeTotal * (Number(serviceFees.hostValue) / 100);
							} else {
							...
							if (serviceFees.guestType === 'percentage') {
								guestServiceFee = priceIncludeTotal * (Number(serviceFees.guestValue) / 100);
							} else {
