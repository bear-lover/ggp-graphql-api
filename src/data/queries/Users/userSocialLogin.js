        ...
        try {
            if (!request.user) {
                ...
                if (!email) {
                    return {
                        status: 400,
                        errorMessage: 'Oops! Unable to find an email address from your [Apple/Facebook] account. The email address is primary need for our application'
                    }
                }
                ...
                if (checkUser) { // Login                     
                    ...
                    if (checkUser.userBanStatus == 1) {
                        ...
                        if (checkUser && checkUser.type === signUpType) {
                            ...
                            return {
                                errorMessage: 'You have already signed up with your ' + capitalizeFirstLetter(checkUser.type) + ' account. Please use your ' + capitalizeFirstLetter(checkUser.type) + ' account for login.',
                                status: 400
                            };
                        }
                    }
                    ...
                    if (newUser) {
                        ...
                        const { status, errorMessage } = sendEmail(newUser.email, 'welcomeEmail', content);
                        return {
                            result: {
                                email: newUser.email,
                                userId: newUser.id,
